          ...
          @include('admin.layouts.partials.alerts')
            ...
            <form @submit.prevent="submitForm" id="addjobformwizard">
              ...
              <div class="flex space-x-4 mt-4">
                <div class="flex-1">
                  <label for="jobTitle" class="block mb-2"
                ...
                <div class="flex-1">
                  ...
                  <select
                    x-ref="laborCategory "  
                    name="cat_id"
                    x-model="formData.laborCategory"
                    class="w-full select2-single custom-style"
                    data-field="laborCategory"
                    id="laborCategory"
                  >
                    <option value="">Select labor category</option>
                    @foreach (getTitles() as $key => $value)
                        <option value="{{ $key }}">{{ $value }}</option>
                    @endforeach
                  </select>
                  <p
                    x-show="showErrors && !isFieldValid('laborCategory')"
                    class="text-red-500 text-sm mt-1"
                    x-text="getErrorMessageById('laborCategory')"
                  ></p>
                </div>
                <div class="flex-1">
                  <label class="block mb-2"
                  ...
                  >
                  <select
                    x-ref="profileWorkerType"
                    x-model="formData.profileWorkerType"
                    name="profile_worker_type_id"
                    class="w-full select2-single custom-style"
                    data-field="profileWorkerType"
                    id="profileWorkerType"
                  >
                    <option value="">Select profile worker type</option>
                    @foreach (profileWorkerType() as $key => $value)
                        <option value="{{ $key }}">{{ $value }}</option>
                    @endforeach
                  </select>
                  <p
                    x-show="showErrors && !isFieldValid('profileWorkerType')"
                    class="text-red-500 text-sm mt-1"
                    x-text="getErrorMessageById('profileWorkerType')"
                  ></p>
                </div>
              </div>
              <!-- Second row -->
              <div class="flex space-x-4 mt-4">
                <div class="flex-1">
                  ...
                  <p
                    x-show="showErrors && !isFieldValid('workerType')"
                    class="text-red-500 text-sm mt-1"
                    x-text="getErrorMessageById('workerType')"
                  ></p>
                </div>
                <div class="flex-1">
                  <label for="jobCode" class="block mb-2"
                    >Job Code <span class="text-red-500">*</span></label
                  >
                  <input
                    type="text"
                     name="job_code"
                    x-model="formData.jobCode"
                    class="w-full h-12 px-4 text-gray-500 border border-gray-300 rounded-md shadow-sm focus:outline-none"
                    value="106207"
                    placeholder="106207"
                ...
                <div class="flex-1">
                  <label class="block mb-2"
                    >Job Family <span class="text-red-500">*</span></label
                  >
                  <select
                    x-ref="jobFamily"
                     name="job_family_id"
                    x-model="formData.jobFamily"
                    class="w-full select2-single custom-style"
                    data-field="jobFamily"
                    id="jobFamily"
                  >
                  ...
                  <p
                    x-show="showErrors && !isFieldValid('jobFamily')"
                    class="text-red-500 text-sm mt-1"
                    x-text="getErrorMessageById('jobFamily')"
                  ></p>
                </div>
              </div>
              <!-- Third row -->
              <div class="flex space-x-4 mt-4">
                <div class="flex-1">
                  ...
                  <select
                   name="status"
                    x-ref="jobCatalogStatus "
                    x-model="formData.jobCatalogStatus"
                    class="w-full select2-single custom-style"
                    data-field="jobCatalogStatus"
                    id="jobCatalogStatus"
                  >
                  ...
                  <p
                    x-show="showErrors && !isFieldValid('jobCatalogStatus')"
                    class="text-red-500 text-sm mt-1"
                    x-text="getErrorMessageById('jobCatalogStatus')"
                  ></p>
                </div>
              ...
              <div class="mt-4">
                ...
                <p
                  x-show="showErrors && !isFieldValid('jobDescription')"
                  class="text-red-500 text-sm mt-1"
                  x-text="getErrorMessageById('jobDescription')"
                ></p>
              </div>
              ...
              <div
                x-data="jobCatalogRateCard()"
                x-init="init"
                @reset-rate-card.window="resetEntries"
              >
                <div class="bg-white p-6 rounded-lg shadow-md mb-6">
                  <div class="flex flex-wrap -mx-3 mb-4">
                    <div class="w-full md:w-1/4 px-3 mb-6 md:mb-0">
                      ...
                      <div class="relative">
                        ...
                        @foreach (jobLevelsList() as $key => $value)
                            <option value="{{ $key }}">{{ $value }}</option>
                        @endforeach
                        </select>
                        <div
                    ...
                    <div class="w-full md:w-1/4 px-3 mb-6 md:mb-0">
                      ...
                      <p
                        ...
                        x-show="errors.minBillRate"
                        x-text="errors.minBillRate"
                      ></p>
                    </div>
                    <div class="w-full md:w-1/4 px-3 mb-6 md:mb-0">
                      <label class="block mb-2">
                      ...
                      <input
                        x-model="newEntry.maxBillRate"
                        name="temp_min_billrate[]"
                      ...
                      <p
                        class="text-red-500 text-xs italic mt-1"
                        x-show="errors.maxBillRate"
                        x-text="errors.maxBillRate"
                      ></p>
                    </div>
                    <div class="w-full md:w-1/4 px-3 mb-6 md:mb-0">
                      ...
                      <div class="relative">
                        <select
                          x-model="newEntry.currency" name="currency"
                          class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                          id="currency"
                        >
                  ...
                  <div class="flex justify-end">
                    <button
                      @click.prevent="addEntry(); $dispatch('rate-card-updated', entries)"
                      ...
                      <template x-for="(entry, index) in entries" :key="index">
                        <tr>
                          <td
                            ...
                            x-text="getJobLevelText(entry.jobLevel)"
                          ></td>
                          <td
                            class="py-3 px-4 border-b border-gray-200"
                            x-text="formatCurrencyDisplay(entry.minBillRate, entry.currency)"
                          ></td>
                          <td
                            class="py-3 px-4 border-b border-gray-200"
                            x-text="formatCurrencyDisplay(entry.maxBillRate, entry.currency)"
                          ></td>
                          <td class="py-3 px-4 border-b border-gray-200">
                            <i
                              :class="getCurrencyIcon(entry.currency)"
                              class="mr-1"
                            ></i>
