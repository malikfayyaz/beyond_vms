...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('job_templates', function (Blueprint $table) {
            ...
            $table->foreignId('cat_id')->constrained('generic_data')->onDelete('cascade');
            $table->text('job_description')->nullable();
            $table->enum('status', ['', 'Active', 'Inactive']);
            // $table->unsignedInteger('currency');
            // $table->foreignId('second_level_id')->constrained('generic_data')->onDelete('cascade');
            $table->foreignId('worker_type_id')->constrained('generic_data')->onDelete('cascade');
            $table->foreignId('profile_worker_type_id')->constrained('generic_data')->onDelete('cascade');
